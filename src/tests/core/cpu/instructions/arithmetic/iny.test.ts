...
describe("INY instruction integration tests", () => {
  ...
  it("INY increments Y and updates flags, consumes 2 cycles", () => {
    // Valor normal
    cpu = createCPU(new FakeRom([Opcode.INCREMENT_Y_REGISTER]));
    ...
    // Zero flag
    cpu = createCPU(new FakeRom([Opcode.INCREMENT_Y_REGISTER], 0x8001));
    ...
    // Negative flag (bit 7)
    cpu = createCPU(new FakeRom([Opcode.INCREMENT_Y_REGISTER], 0x8002));
