...
describe("INX instruction integration tests", () => {
  ...
  it("INX increments X and updates flags, consumes 2 cycles", () => {
    // Valor normal
    cpu = createCPU(new FakeRom([Opcode.INCREMENT_X_REGISTER]));
    ...
    // Zero flag
    cpu = createCPU(new FakeRom([Opcode.INCREMENT_X_REGISTER], 0x8001));
    ...
    // Negative flag (bit 7)
    cpu = createCPU(new FakeRom([Opcode.INCREMENT_X_REGISTER], 0x8002));
