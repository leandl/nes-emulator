...
describe("LDA instruction integration tests", () => {
  ...
  it("LDA immediate loads accumulator and sets flags correctly", () => {
    // Valor normal
    loadProgram([Opcode.LOAD_ACCUMULATOR_IMMEDIATE, 0x42]);
    cpu.step();
    ...
    // Zero flag
    loadProgram([Opcode.LOAD_ACCUMULATOR_IMMEDIATE, 0x00]);
    cpu.step();
    ...
    // Negative flag
    loadProgram([Opcode.LOAD_ACCUMULATOR_IMMEDIATE, 0x80]);
    cpu.step();
  ...
  it("LDA zero page loads accumulator", () => {
    ...
    loadProgram([Opcode.LOAD_ACCUMULATOR_ZERO_PAGE, 0x10]);
    cpu.step();
  ...
  it("LDA zero page X loads accumulator with X offset", () => {
    ...
    loadProgram([Opcode.LOAD_ACCUMULATOR_ZERO_PAGE_X, 0x10]);
    cpu.step();
  ...
  it("LDA absolute loads accumulator", () => {
    ...
    loadProgram([Opcode.LOAD_ACCUMULATOR_ABSOLUTE, 0x00, 0x20]);
    cpu.step();
  ...
  it("LDA absolute X loads accumulator", () => {
    ...
    loadProgram([Opcode.LOAD_ACCUMULATOR_ABSOLUTE_X, 0x00, 0x20]);
    cpu.step();
  ...
  it("LDA absolute Y loads accumulator", () => {
    ...
    loadProgram([Opcode.LOAD_ACCUMULATOR_ABSOLUTE_Y, 0x00, 0x20]);
    cpu.step();
    expect(cpu.registers.A).toBe(0x77);
  });
  ...
  it("LDA (indirect,X) loads accumulator", () => {
    ...
    loadProgram([Opcode.LOAD_ACCUMULATOR_INDIRECT_X, 0x10]);
    cpu.step();
  ...
  it("LDA (indirect),Y loads accumulator", () => {
    ...
    loadProgram([Opcode.LOAD_ACCUMULATOR_INDIRECT_Y, 0x10]);
    cpu.step();
    expect(cpu.registers.A).toBe(0x55);
  });
});
